...
crazy_guest = True
crazy_vent = False 
crazy_storage = False #Эти переменные отвечают за уборщика(cleaner_go за то пойдет он или нет а остально за то в какой комнате он)
...
def start(message):
    db_manager.add_user(message.from_user.username, '0', '0', 'reg_user')
...
def help(message):
    ...
    if status == 'admin':
        bot.reply_to(message, "Комманды разработчика: /win - мгновенная победа \n  /kill - мгновенный проигрыш")
    else:
        bot.reply_to(message, "У вас нет прав использовать команды разработчика!")
...
def win(message):
    ...
    bot.send_message(message.chat.id, "Вы победили! (+1 победа, +10 монет)", reply_markup=markup)
...
def timing_thread():
    global timer, go_cleaner, go_hoverboard, go_сrazy, go_barmen, night
    while True:
        if game_started != True:
            None 
        else:
